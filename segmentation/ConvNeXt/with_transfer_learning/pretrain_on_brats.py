...
Task: Fusion vs mutation vs other
Train RF fusion model using both CNN and radiomic features together
"""
import numpy as np
import pandas as pd
import torch
...
if __name__ == "__main__":
    ...
    dev_mode = True  # if true, use just a small number of patients, to quickly run through and make sure everything is working as expected
    ...
    for t in range(num_trials):
        ...
        for hyper_counter in range(len(list(seg_param_grid))):
            ...
            for epoch in range(seg_epochs):
                ...
                for batch_idx, (idx) in enumerate(train_dataloader):
                    ...
                    bce_loss, dsc_loss = criterion_seg(output_seg, label_mask.float())
                ...
                if val_loss< seg_lowest_val_loss_overall:
                    seg_best_model = copy.deepcopy(net)
            ...
            torch.save(seg_best_model.state_dict(), model_dir + "/pretrained_seg_model"+str(t)+".pt")
        ...
        name = "mednext_brats_results_t" + str(t+1) + name
        print(name)
        results.to_csv(name)
