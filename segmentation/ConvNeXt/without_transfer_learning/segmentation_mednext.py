    ...
    for t in range(num_trials):
            ...
            for epoch in range(seg_epochs):
                ...
                with torch.set_grad_enabled(False):
                    ...
                    for batch_idx, (idx) in enumerate(test_dataloader):
                        ...
                        test_ids += [patient_ids[i] for i in idx.numpy().tolist()]
                ...
                if val_loss< seg_lowest_val_loss_overall:
                    ...
                    seg_best_segmentation_ids = test_ids
        ...
        if save_segmentations:
            if t>0:
                ...
                with open('segs_ids_'+name[:-3] +".npy", 'rb') as f:
                    last_segs_ids = np.load(f)
                ...
                os.remove('segs_ids_'+name[:-3]+".npy")
                print(f"Removed file {'segs_id_'+name[:-3]+'.npy'}")
        ...
        name = "mednext_plgg_results_t" + str(t+1) + name
        print(name)
        results.to_csv(name)
        ...
        if save_segmentations:
            ...
            seg_best_segmentation_ids = np.array(seg_best_segmentation_ids)
            if t>0:
                print(last_segs_pred.shape)
                print(last_segs_true.shape)
                print(last_segs_ids.shape)
                ...
                seg_best_segmentation_ids = np.concatenate((last_segs_ids, seg_best_segmentation_ids))
            print(seg_best_segmentation_pred.shape)
            print(seg_best_segmentation_label.shape)
            print(seg_best_segmentation_ids.shape)
            ...
            with open('segs_ids_'+name[:-3]+".npy", 'wb') as f:
                np.save(f, seg_best_segmentation_ids)
