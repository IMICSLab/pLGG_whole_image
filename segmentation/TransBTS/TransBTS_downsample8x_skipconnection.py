...
class TransformerBTS(nn.Module):
    def __init__(
        ...
        self.Unet = Unet(in_channels=1, base_channels=2, num_classes=1)
...
class EnBlock1(nn.Module):
    def __init__(self, in_channels):
        ...
        self.bn1 = nn.BatchNorm3d(64 // 4)
        self.relu1 = nn.ReLU(inplace=True)
        self.bn2 = nn.BatchNorm3d(64 // 4)
...
def TransBTS(dataset='brats', _conv_repr=True, _pe_type="fixed"):
...
if __name__ == '__main__':
    with torch.no_grad():
        ...
        x = torch.rand((2, 1, 240, 240, 155), device=cuda0)
        print(x.shape)
        _, model = TransBTS()
        model.cuda()
        y = model(x)
