...
class ResNet(nn.Module):
    ...
    def __init__(self,
        ...
        self.relu = nn.ReLU(inplace=True)
        self.maxpool = nn.MaxPool3d(kernel_size=(3, 3, 3), stride=2, padding=0)
        self.layer1 = self._make_layer(block, 8, layers[0], shortcut_type)
        self.layer2 = self._make_layer(
            block, 16, layers[1], shortcut_type, stride=1)
        self.layer3 = self._make_layer(
            block, 32, layers[2], shortcut_type, stride=1, dilation=2)
        self.layer4 = self._make_layer(
            block, 64, layers[3], shortcut_type, stride=1, dilation=4)
...
if __name__ == "__main__":
    ...
    for t in range(num_trials):
        ...
        for hyper_counter in range(len(list(seg_param_grid))):
            ...
            net = ResNet(BasicBlock, [1, 1, 1, 1], 1, fc_dropout_rate=seg_fc_dropout_rate, cnn_dropout_rate=seg_conv_dropout_rate,
                   seg=True)
        ...
        name = "medicalnet_plgg_results_t" + str(t+1) + name
        print(name)
        results.to_csv(name)
