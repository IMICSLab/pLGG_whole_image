...
def load_data_for_patient(patient_id):
    # Get label and radiomics input
    label = training_labels[patient_id]
    if label == "HGG":
        label = 1
    elif label == "LGG":
        label = 0
    else:
        exit("Problem with parsing labels")
    label = torch.tensor(label).float().unsqueeze(0)

    file_name=patient_id

    # Load the segmentation data
    mask = nib.load(os.path.join(data_dir + "/" + file_name+"/"+file_name+"_seg.nii.gz")).get_fdata()

    mask = torch.tensor(mask).float().unsqueeze(0)
    mask = torch.where(mask > 0, 1.0, 0.0)
    flair_image = nib.load(os.path.join(data_dir + "/" + file_name+"/"+file_name+"_flair.nii.gz")).get_fdata()
...
if __name__ == "__main__":
    ...
    if socket.gethostname()=='RT6248W-NMH':
        df_data_labels = pd.read_csv(
            "C:/Users/kareem kudus/Documents/brats2020/MICCAI_BraTS2020_TrainingData/name_mapping.csv")
        data_dir = "C:/Users/kareem kudus/Documents/brats2020/MICCAI_BraTS2020_TrainingData"
        model_dir = "./"
    else:
        df_data_labels = pd.read_csv("/hpf/largeprojects/fkhalvati/Datasets/MedicalImages/BrainData//BraTS2020//MICCAI_BraTS2020_TrainingData/name_mapping.csv")
        data_dir = "/hpf/largeprojects/fkhalvati/Datasets/MedicalImages/BrainData//BraTS2020//MICCAI_BraTS2020_TrainingData"
        model_dir = "./"
    ...
    #######################################Preprocessing######################################
    training_labels = dict(zip(df_data_labels["BraTS_2020_subject_ID"].values, df_data_labels["Grade"].values))
    print(training_labels)

    if dev_mode == True:
        keys = list(training_labels.keys())
        random.shuffle(keys)
        training_labels=dict([(key, training_labels[key]) for key in keys])
    ...
    print(data_masks.min())
    print(data_masks.max())
    ...
    for t in range(num_trials):
        ...
        for hyper_counter in range(len(list(class_param_grid))):
            ...
            if dev_mode != True:
                loss_weights = torch.tensor([
                    ...
                    num_patients_total/temp_labels[temp_labels == 1].size(dim=0)]).to(device)
                loss_weights = loss_weights/torch.sum(loss_weights)
            else:
                loss_weights = torch.tensor([1.0,1.0]).to(device)
            ...
            for epoch in range(class_epochs):
                ...
                for batch_idx, (idx) in enumerate(train_dataloader):
                    ...
                    for i in range(len(label.tolist())):
                        ...
                        training_estimated.append(output[i].cpu().detach().numpy())
                ...
                with torch.set_grad_enabled(False):
                    ...
                    for batch_idx, (idx) in enumerate(validation_dataloader):
                        ...
                        for i in range(len(label.tolist())):
                            ...
                            validation_estimated.append(output[i].cpu().detach().numpy())
                        loss = criterion(output, label)
                    ...
                    for batch_idx, (idx) in enumerate(test_dataloader):
                        ...
                        for i in range(len(label.tolist())):
                            ...
                            test_estimated.append(output[i].cpu().detach().numpy().tolist())
                            test_estimated_label.append(torch.argmax(output[i]).cpu().detach().numpy())
                ...
                # test_auc_breakdown = roc_auc_score(test_true, test_estimated)
        ...
        torch.save(class_best_model.state_dict(), model_dir + "/pretrained_model" + str(t) + ".pt")
        ...
        name = name + "_classauc" + str(np.round(np.mean(class_test_AUCs), 3)) + "_classacc" +str(np.round(np.mean(class_test_accs),3)) +\
               ...
               "_" + timestamp + ".csv"
        name = "brats_whole_image_pretrain_"+ "_results" + "_t" + str(t+1) + name
        print(name)
        results.to_csv(name)
